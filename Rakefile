...
def getMonthGenitiveEng(m)
  case m 
  when 1 then 'ianvaria'
  when 2 then 'fievralia'
  when 3 then 'marta'
  when 4 then 'aprielia'
  when 5 then 'maia'
  when 6 then 'iiunia'
  when 7 then 'iiulia'
  when 8 then 'avgusta'
...
task :today, :param do |t, args|
  ...
  puts "cDate="<<cDate.strftime('%Y-%m-%d')
  raise "### You haven't set anything up yet. First run `rake install` to set up an Octopress theme." unless File.directory?(source_dir)
  mkdir_p "#{source_dir}/#{posts_dir}"
  #filename = "#{source_dir}/#{posts_dir}/#{Time.now.strftime('%Y-%m-%d')}-#{title.to_url}.#{new_post_ext}"
  m=cDate.strftime('%m').to_i
	s=getMonthGenitiveEng(m)
  sr=getMonthGenitiveRus(m)
  ...
  open(filename, 'w') do |post|
    ...
    post.puts "layout: post"
    post.puts "title: \"#{title.gsub(/&/,'&amp;')}\""
    post.puts "date: #{cDate.strftime('%Y-%m-%d %H:%M:%S %z')}"
    post.puts "comments: true"
    post.puts "categories: "
    post.puts "---"
  end
